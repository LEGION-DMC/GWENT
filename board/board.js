const boardModule = {
	...
	initAbilityIcon: function() {
		...
		if (abilityData && abilityIconImg) {
			// Используем правильный путь к иконке
			if (abilityData.icon) {
	...
	getAbilityData: function(factionId, abilityId) {
		...
		// 1. Проверяем factionAbilities из deck.js
		...
		// 3. Пробуем найти иконку по имени способности
		const iconName = this.getAbilityIconName(abilityId, factionId);
		return {
			id: abilityId,
	...
	getAbilityIconName: function(abilityId, factionId) {
		...
		// Маппинг ID способностей на имена файлов иконок
		const iconMap = {
			// Скоя'таэли
	...
	getAbilityDescription: function(abilityId) {
		const descMap = {
			...
			'monsters_ability_2': 'Уничтожьте дружественный отряд, затем создайте в этом ряду Волколака и усильте его на значение силы уничтоженного отряда',
    ...
    handleAbilityClick: function() {
        ...
        if (abilityIcon.classList.contains('ability-used')) {
            if (window.gameModule?.showGameMessage) {
    ...
    updateAbilityAvailability: function(gameState) {
        ...
        const isUsed = gameState?.player?.abilityUsedThisRound || false;
        const isPlayerTurn = gameState?.currentPlayer === 'player' && gameState?.gamePhase === 'playerTurn';
        const isPassed = gameState?.player?.passed || false;
			...
			<div class="player-leader-area leader-area">
				<div class="leader-slot" id="playerLeader">
					...
					<div class="leader-ability-wrapper" id="playerAbilityWrapper">
						<div class="leader-ability-icon" id="playerAbilityIcon">
